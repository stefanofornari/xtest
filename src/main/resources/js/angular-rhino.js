...
// RHINO
/**
 * TODO: displaying errorrs in the console
 */
// ----
...
var trim = function(value) {
  return isString(value) 
       ? ((value.trim) ? value.trim() : value.replace(/^\s+|\s+$/gm,""))
       : value;
};
// ---
...
function parseAbsoluteUrl(absoluteUrl, locationObj) {
  ...
  locationObj.$$port = int10(parsedUrl.port) || DEFAULT_PORTS[parsedUrl.protocol] || null;
}
...
function $SnifferProvider() {
  this.$get = ['$window', '$document', function($window, $document) {
    var eventSupport = {},
        android =
          int10((/android (\d+)/.exec(lowercase(($window.navigator || {}).userAgent)) || [])[1]),
...
function dateFilter($locale) {
  ...
  function jsonStringToDate(string) {
    ...
    if (match = string.match(R_ISO8601_STR)) {
      ...
      if (match[9]) {
        tzHour = int10(match[9] + match[10]);
        tzMin = int10(match[9] + match[11]);
      }
      dateSetter.call(date, int10(match[1]), int10(match[2]) - 1, int10(match[3]));
      var h = int10(match[4] || 0) - tzHour;
      var m = int10(match[5] || 0) - tzMin;
      var s = int10(match[6] || 0);
  ...
  return function(date, format, timezone) {
    ...
    if (isString(date)) {
      date = NUMBER_STRING.test(date) ? int10(date) : jsonStringToDate(date);
    }
